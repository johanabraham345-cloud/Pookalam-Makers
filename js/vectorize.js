(function (g) {
  ...
  function classifyPath(ptsN, tol) {
    const first = FIT.classify(ptsN, tol);
    if (!first) return [];
    if (first.kind !== 'poly') return [first];

    const idx = TR.simplifyIdx(ptsN, tol * 1.3);
    if (idx.length < 3) return [first];
    const out = [];
  ...
  VEC.run = (srcCanvas, crop, opt) => {
    ...
    const cx = W / 2, cy = W / 2, R = W / 2 * 0.94;
    ...
    const norm = paths.map(p => p.map(q => [(q[0] - cx) / R, (q[1] - cy) / R]));

    const fits = [];
    for (const p of norm) {
      for (const f of classifyPath(p, opt.tol)) fits.push(f);
    }
