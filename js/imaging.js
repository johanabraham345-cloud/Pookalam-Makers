(function (g) {
  ...
  IM.guessFolds = (m, cx, cy, R) => {
    ...
    let best = -1, bestN = 1;
    for (let n = 2; n <= 24; n++) {
      const s = IM.profileCorr(p, NA / n);
      scores[n] = s;
      if (s > best) { best = s; bestN = n; }
    }
    if (best < 0.45) return { n: 1, score: best, scores: scores };
    for (let n = 24; n > bestN; n--) {
      if (scores[n] >= best - 0.04) { bestN = n; break; }
    }
    return { n: bestN, score: scores[bestN], scores: scores };
  };
