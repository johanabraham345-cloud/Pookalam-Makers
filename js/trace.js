(function (g) {
  ...
  TR.tracePaths = skel => {
    ...
    for (let i = 0; i < d.length; i++) {
      ...
      for (const nb of nbrsOf(i)) {
        if (usedEdge.has(key(i, nb))) continue;
        const p = walk(i, nb);
    ...
    for (let i = 0; i < d.length; i++) {
      if (!d[i] || deg[i] !== 2) continue;
      let fresh = -1;
      for (const nb of nbrsOf(i)) if (!usedEdge.has(key(i, nb))) { fresh = nb; break; }
      if (fresh < 0) continue;
    ...
    const chainFrom = startId => {
      ...
      for (let guard = 0; guard < paths.length + 2; guard++) {
        ...
        if (pts.length) {
          ...
          if (Math.hypot(seg[0][0] - last[0], seg[0][1] - last[1]) > dTol) break;
        }
        used[i] = 1;
      ...
      if (closed && pts.length > 2) {
        ...
        if (Math.hypot(a[0] - b[0], a[1] - b[1]) <= dTol) pts.push(a);
      }
      return pts;
    };
